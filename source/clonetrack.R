...
plot_clonetracks.fx <- function(compldfle, plotpath, pattern, chain, countfrac, clnefrc){
  ...
  myp <- p + geom_alluvium(decreasing = FALSE) +
    ...
    labs(y = countfrac)
  pdf(paste0(plotpath, "clonetrack_",
             pattern,"_",chain,".pdf"),
      width = 15,
      height = 20,
      useDingbats = FALSE,
...
pt_df <- cdr3_dataframe.fx(datapath, type, flelst[grepl(pattern, flelst)], "inframe")

plot_clonetracks.fx(pt_df, datapath,pattern, type, "cloneFraction", 0) 
